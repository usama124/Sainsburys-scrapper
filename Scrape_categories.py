...
def get_categories_tags(page_obj):
    ...
    for tag in tags_ul:
        ...
        tags_concat.append(tag_text)
    while len(tags_concat) < 4:
        tags_concat.append("")
...
def find_weight_from_title(title : str):
    ...
    for sp_t in splitted_title:
        sp_t = sp_t.split("x")[-1]
        if "g" in sp_t.lower() or "kg" in sp_t.lower():
...
def scrape_product(link, selenium_webdriver):
    ...
    try:
        ...
        categories_list = get_categories_tags(page_obj)
...
def click_no_thanks(selenium_driver):
    try:
        selenium_driver.webdriver.find_element_by_id("").click()
    except:
        pass
...
def scrape_products_page(link, selenium_webdriver, list_scraped_products):
    ...
    while True:
        ...
        try:
            selenium_webdriver.webdriver.find_element_by_css_selector(
                '#productLister > div:nth-child(2) > ul.pages > li.next > a').click()
            time.sleep(2)
            counter = 0
            break
        except Exception as e:
